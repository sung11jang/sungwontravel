...
export default function App() {
  ...
  const [theme, setTheme] = useState('dark'); // 'dark' | 'light' | 'vivid'
  ...
  const styles = useMemo(() => {
    if (theme === 'light') {
      return {
        ...
        header: 'bg-white/95 border-b border-slate-300 text-slate-900',
        card: 'bg-white border-slate-300 shadow-sm text-slate-900',
        innerCard: 'bg-slate-50 border-slate-300 text-slate-900',
        hudBox: 'bg-white border-2 border-slate-300 text-slate-900 shadow-sm',
        nav: 'bg-white/95 border-t border-slate-300 text-slate-600',
        navActive: 'text-blue-700 font-extrabold bg-blue-100/70 border border-blue-300',
        accentText: 'text-blue-700',
        subText: 'text-slate-600',
        badge: 'bg-blue-100 text-blue-900 border-blue-300 font-bold',
        btnPrimary: 'bg-blue-600 hover:bg-blue-500 text-white font-bold shadow',
        input: 'bg-white border-slate-400 text-slate-900 focus:border-blue-600'
      };
    } else if (theme === 'vivid') {
      return {
        bg: 'bg-slate-950 text-white',
        header: 'bg-cyan-950/95 border-b-2 border-cyan-400 text-white',
        card: 'bg-slate-900 border-2 border-cyan-400 shadow-xl text-white',
        innerCard: 'bg-slate-950 border border-cyan-500/60 text-slate-100',
        hudBox: 'bg-slate-900 border-2 border-cyan-400 text-white shadow-lg',
        nav: 'bg-slate-950/95 border-t-2 border-cyan-400 text-slate-300',
        navActive: 'text-cyan-300 font-black bg-cyan-950 border-2 border-cyan-400',
        accentText: 'text-cyan-300 font-black',
        subText: 'text-slate-300',
        badge: 'bg-cyan-500/30 text-cyan-200 border border-cyan-300 font-black',
        btnPrimary: 'bg-gradient-to-r from-cyan-400 to-blue-500 text-slate-950 font-black shadow-lg',
        input: 'bg-slate-900 border-2 border-cyan-400 text-white focus:border-cyan-200'
      };
    }
    // Default: Dark Mode
    return {
      ...
      card: 'bg-slate-800/90 border-slate-700 shadow-lg text-slate-100',
      innerCard: 'bg-slate-900/80 border-slate-700/80 text-slate-200',
      hudBox: 'bg-slate-800/95 border border-slate-700 text-slate-100 shadow-md',
  ...
  return (
    <div className={`min-h-screen ${styles.bg} font-sans pb-32 select-none antialiased transition-colors duration-150`}>
      {/* ── TOP HUD HEADER (대형 폰트 적용) ── */}
      <header className={`sticky top-0 z-40 ${styles.header} backdrop-blur-md px-4 py-3.5 shadow-md`}>
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-2">
          <div className="flex items-center space-x-2.5">
            <span className="text-2xl">🌴</span>
            <div>
              <h1 className={`text-lg sm:text-xl font-extrabold ${styles.accentText} leading-tight`}>
                푸꾸옥 가족여행 4박5일
              </h1>
              <p className={`text-xs sm:text-sm font-semibold ${styles.subText}`}>12/12(금) ~ 12/16(화) • 성원 가족</p>
            </div>
          </div>

          {/* Theme Switcher & Tip Modal */}
          <div className="flex items-center space-x-2">
            <div className="flex items-center bg-slate-800/40 p-1 rounded-xl border border-slate-600/50">
              <Palette className="w-4 h-4 mr-1 text-amber-400 shrink-0" />
              <button
                onClick={() => setTheme('dark')}
                className={`px-2 py-1 rounded text-xs font-bold ${theme === 'dark' ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}
              >
                다크
              </button>
              <button
                onClick={() => setTheme('light')}
                className={`px-2 py-1 rounded text-xs font-bold ${theme === 'light' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
              >
                라이트
              </button>
              <button
                onClick={() => setTheme('vivid')}
                className={`px-2 py-1 rounded text-xs font-black ${theme === 'vivid' ? 'bg-cyan-400 text-slate-950' : 'text-slate-400'}`}
              >
                비비드
              </button>
            </div>
            ...
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-2 rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/40 text-xs sm:text-sm font-bold hover:bg-amber-500/30 transition-all active:scale-95"
            >
              <Sparkles className="w-4 h-4 text-amber-400" />
              <span className="font-bold">꿀팁</span>
            </button>
          </div>
        </div>

        {/* HUD Quick Info Strip (글자 대폭 확대) */}
        <div className={`max-w-3xl mx-auto mt-3 grid grid-cols-3 gap-2.5 ${styles.hudBox} p-3 rounded-2xl`}>
          {/* 1. Dual Clock */}
          <div className="flex flex-col items-center justify-center border-r border-slate-600/40 pr-1">
            <span className={`text-xs font-bold flex items-center gap-1 mb-0.5 ${styles.subText}`}>
              <Clock className="w-3.5 h-3.5 text-cyan-400" /> 시계
            </span>
          ...
          <div className="flex flex-col items-center justify-center border-r border-slate-600/40 pr-1">
            <span className={`text-xs font-bold flex items-center gap-1 mb-0.5 ${styles.subText}`}>
              <CloudSun className="w-3.5 h-3.5 text-amber-400" /> 날씨
            </span>
            <div className="text-center mt-0.5">
              <span className="text-base sm:text-lg font-black text-amber-400 leading-tight">{weather.temp}°C</span>
          ...
          <div className="flex flex-col items-center justify-center">
            <span className={`text-xs font-bold flex items-center gap-1 mb-0.5 ${styles.subText}`}>
              <DollarSign className="w-3.5 h-3.5 text-emerald-400" /> 간이 환율
            </span>
            <div className="text-center mt-0.5">
              <span className="text-sm sm:text-base font-black text-emerald-400 leading-tight">동 ÷ 20</span>
      ...
      <main className="max-w-3xl mx-auto px-4 pt-4">
        ...
        {activeTab === 'timeline' && (
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className={`${styles.card} p-4 rounded-2xl border flex items-center justify-between`}>
              ...
              <span className={`px-2.5 py-1 rounded-lg text-xs sm:text-sm font-bold border ${styles.badge} shrink-0`}>
                수하물 20kg
              </span>
            </div>
            ...
            <div className="space-y-4">
              {scheduleData.map((dayData, idx) => (
                <div key={idx} className={`${styles.card} rounded-2xl border p-4 sm:p-5`}>
                  {/* Day Header */}
                  <div className="flex items-center justify-between pb-3.5 mb-3.5 border-b border-slate-600/30">
                    <div>
                    ...
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border ${styles.badge} shrink-0`}>
                      {dayData.badge}
                    </span>
                  </div>

                  {/* Timeline Items */}
                  <div className="relative pl-5 space-y-5 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-600/40">
                    {dayData.items.map((item, itemIdx) => (
                      <div key={itemIdx} className="relative group">
                        {/* Dot */}
                        <div className="absolute -left-[23px] top-1.5 w-3.5 h-3.5 rounded-full bg-emerald-500 ring-4 ring-slate-800" />
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {item.mapQuery && (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className={`shrink-0 px-2.5 py-1.5 rounded-xl ${styles.innerCard} border hover:border-emerald-500 transition-colors flex items-center justify-center text-xs font-bold gap-1 mt-1`}
                            >
        ...
        {activeTab === 'routes' && (
          <div className="space-y-5 animate-fadeIn">
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-sm sm:text-base font-bold text-amber-400">💡 권역별 최적 이동수단 안내 (추천 옵션)</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border`}>
                <span className={`text-xs font-bold px-2.5 py-1 rounded border ${styles.badge}`}>Day 1~2</span>
                ...
                <div className={`mt-3 pt-3 border-t border-slate-600/30 text-xs sm:text-sm font-bold ${styles.accentText}`}>
                  추천: 중부 숙소 거점 이동
                </div>
              </div>

              <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border`}>
                <span className={`text-xs font-bold px-2.5 py-1 rounded border ${styles.badge}`}>Day 1~3 베이스</span>
              ...
              <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border`}>
                <span className={`text-xs font-bold px-2.5 py-1 rounded border ${styles.badge}`}>Day 3~5 베이스</span>
            ...
            {/* Routes Detailed Box */}
            <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border space-y-3.5`}>
              ...
              <div className="space-y-3">
                <div className={`${styles.innerCard} p-4 rounded-xl border`}>
        ...
        {activeTab === 'vouchers' && (
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border space-y-3.5`}>
              <h3 className="text-base sm:text-lg font-bold flex items-center justify-between">
                <span className="flex items-center gap-2">
                  <Bus className="w-5 h-5 text-cyan-400" /> 교통 & 셔틀 예약 관리 현황
                </span>
              ...
              <div className="space-y-2.5">
                ...
                <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-start gap-3">
                  ...
                  <div className="text-xs sm:text-sm">
                    <span className="font-bold text-amber-400 text-sm sm:text-base">시쉘 체크아웃 셔틀 현장 신청</span>
                    <span className="ml-2 px-2 py-0.5 rounded text-xs font-bold bg-amber-500/20 text-amber-400">미완료(안한것)</span>
                    <p className={`text-xs sm:text-sm mt-1 leading-relaxed ${styles.subText}`}>
            ...
            {/* Flight Voucher */}
            <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border space-y-2.5`}>
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded border ${styles.badge}`}>VJ977 / VJ976</span>
              </div>
              <div className={`${styles.innerCard} p-3.5 rounded-xl text-xs sm:text-sm space-y-1.5 leading-relaxed`}>
            ...
            <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border space-y-3.5`}>
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded border ${styles.badge}`}>
                  일반 승용차 기준
                </span>
              </div>

              <div className={`${styles.innerCard} p-4 rounded-xl text-xs sm:text-sm space-y-2.5`}>
                <div className="flex justify-between pb-2 border-b border-slate-600/30 text-sm sm:text-base font-bold">
                  <span>예상 요금 (일반 승용차)</span>
                  <span className="text-amber-400">5일 45,000원 (일 9,000원)</span>
                </div>
          ...
          <div className="space-y-5 animate-fadeIn">
            ...
            <div className={`${styles.card} rounded-2xl p-4 sm:p-5 border space-y-3.5`}>
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-mono px-2.5 py-1 rounded border ${styles.badge}`}>
                  1 VND ≈ {exchangeRate.toFixed(4)} KRW
                </span>
              </div>

              <div className={`${styles.innerCard} p-4 sm:p-5 rounded-xl flex flex-col sm:flex-row justify-between items-baseline gap-2`}>
              ...
              <form onSubmit={addExpense} className="grid grid-cols-1 sm:grid-cols-4 gap-2.5 pt-1">
                ...
                <select
                  ...
                  className={`${styles.input} rounded-xl px-3 py-2.5 text-xs sm:text-sm focus:outline-none`}
                >
              ...
              <div className="space-y-2.5 max-h-56 overflow-y-auto pr-1">
                {expenses.map((exp) => (
                  <div key={exp.id} className={`flex items-center justify-between p-3.5 rounded-xl ${styles.innerCard} border text-xs sm:text-sm`}>
                    <div>
                      <span className="font-bold text-sm sm:text-base">{exp.title}</span>
                      <span className={`text-xs ml-2 px-2 py-0.5 rounded border ${styles.badge}`}>
                        {exp.category}
                      </span>
                    </div>
                    <div className="flex items-center space-x-3.5">
                      <div className="text-right">
                        <span className="font-bold text-amber-400 block text-sm sm:text-base">{exp.amountVND.toLocaleString()} VND</span>
                ...
                {checklist.map((item) => (
                  <div
                    ...
                    className={`flex items-start space-x-3 p-3 rounded-xl border transition-all cursor-pointer ${
                      item.checked
                        ? 'opacity-50 line-through border-transparent'
                        : `${styles.innerCard} hover:border-slate-500`
                    }`}
                  >
                    <input
                      type="checkbox"
                    ...
                    />
                    <div className="text-xs sm:text-sm">
                      <span className={`text-xs font-bold px-2 py-0.5 rounded mr-1.5 border ${styles.badge}`}>
                        {item.cat}
                      </span>
        ...
        <div className="max-w-md mx-auto grid grid-cols-4 gap-1.5">
          <button
            ...
            className={`flex flex-col items-center py-2 px-2 rounded-xl transition-all ${
              activeTab === 'timeline' ? styles.navActive : `${styles.subText} hover:text-slate-100`
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center py-2 px-2 rounded-xl transition-all ${
              activeTab === 'routes' ? styles.navActive : `${styles.subText} hover:text-slate-100`
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center py-2 px-2 rounded-xl transition-all ${
              activeTab === 'vouchers' ? styles.navActive : `${styles.subText} hover:text-slate-100`
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center py-2 px-2 rounded-xl transition-all ${
              activeTab === 'budget' ? styles.navActive : `${styles.subText} hover:text-slate-100`
            }`}
          >
      ...
      {showTipModal && (
        <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className={`${styles.card} rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-5 sm:p-6 shadow-2xl space-y-4 border`}>
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-600/30">
              <div className="flex items-center space-x-2.5">
                <Sparkles className="w-6 h-6 text-amber-400" />
                <h3 className="text-base sm:text-lg font-bold">푸꾸옥 여행 실전 필수 꿀팁</h3>
              </div>
              <button onClick={() => setShowTipModal(false)} className="text-slate-400 hover:text-slate-100 text-xl font-bold px-2.5 py-1">
                ✕
              </button>
            </div>
            ...
            <div className="space-y-3.5 text-xs sm:text-sm leading-relaxed">
              <div className={`${styles.innerCard} p-4 rounded-xl border`}>
                <h4 className="font-bold text-amber-400 text-sm sm:text-base mb-1.5">💵 화폐 수취 주의</h4>
              ...
              <div className={`${styles.innerCard} p-4 rounded-xl border`}>
                <h4 className="font-bold text-cyan-400 text-sm sm:text-base mb-1.5">🚖 그랩 호객 방지</h4>
                <p>• 공항 출구 등에서 기사를 자처하는 호객 행위에 응하지 마시고, 앱에 등록된 차량 번호판을 확인 후 탑승하세요.</p>
              </div>

              <div className={`${styles.innerCard} p-4 rounded-xl border`}>
                <h4 className="font-bold text-emerald-400 text-sm sm:text-base mb-1.5">🚿 수질 & 식수 위생</h4>
                <p>• 호텔에서도 샤워기 필터 헤드를 반드시 교체해 사용하시고, 양치 후 마지막 헹굼물은 생수를 권장합니다.</p>
              </div>
            </div>

            <button onClick={() => setShowTipModal(false)} className={`w-full py-3 ${styles.btnPrimary} rounded-xl text-sm font-bold transition-all`}>
              확인 완료 (닫기)
            </button>
          </div>
        </div>
